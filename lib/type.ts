...
// 式の型推論
function infer_exp(env: TypeDict, exp: Exp): Type{
    switch(exp.type){
        case 'unit':
            return make.unit();
        case 'bconst':
        ...
        case 'application': {
            const {exp1, args} = exp;
            let t1 = infer_exp(env, exp1);
            const argst = args.map(e => infer_exp(env, e));
            // 順に適用
        ...
        case 'branch': {
            ...
            const condt = infer_exp(env, cond);
            unify(condt, make.bool());
            const exp1t = infer_exp(env, exp1);
            const exp2t = infer_exp(env, exp2);
            // 型の一致
            unify(exp1t, exp2t);
            return exp1t;
        }
